...
#my_tabular_data_raw <- read_csv(here("lessons", "sf-lesson-20200303" , "data", "tabular_data", "conterm_climate.txt")) # I'm choosing climate data here

gages_raw <- st_read(here("lessons", "sf-lesson-20200303" , "data", "cafo_data", "Points.shp"))
swine_raw <- st_read(here("lessons", "sf-lesson-20200303" , "data", "cafo_data", "Swine_CAFO.shp"))
nc_counties_raw <- read_csv(here("lessons", "sf-lesson-20200303" , "data", "cafo_data", "NC_Counties.shp"))
...
# add your code here
nc_counties <- nc_counties_raw
st_crs(nc_counties) <- my_proj4
...
# set the proj4 string first and then set the EPSG code (because proj4 will over-ride EPSG if you set EPSG first)
nc_state_county_na_albers <-  sf::st_transform(nc_counties, crs = na_albers_proj4) %>%
...
head(nc_counties)
nc_county_geometry <- st_geometry(nc_counties)

# tabular data is dropped
head(nc_county_geometry)

# notice the class is different now
class(nc_county_geometry)
...
# convert gages to WGS84 so it matches nc_basemap
nc_gages <- gages%>%
  st_transform(4326)
st_crs(nc_gages)
...
ggmap(nc_basemap) +
  ...
  geom_sf(data = nc_gages,size = 1, inherit.aes = FALSE)+
  geom_sf(data= swine_nc, size=1, color = "pink", inherit.aes = FALSE)
dev.off()
...
# select NC
# add your code here
nc_state_bounds_geom <- nc_state_bounds_geom_wsg84 %>%
  filter(NAME == "North Carolina") %>%
  st_geometry()
...
# add your code here
nc_gages_overlap_sel <- gages %>%
  st_intersection(nc_basins_overlap_sel)
# stretch break...this might take a while to run

nc_swine <- swine%>%
  st_intersection(nc_state_bounds_geom)
nc_swine_overlap <- swine%>%
  st_intersection(nc_basins_overlap_sel)

nc_counties_f <- nc_counties%>%
  st_interection(nc_state_bounds_geom)
nc_counties_overlap <-nc_counties%>%
  st_intersection(nc_basins_overlap_sel)
...
nc_gages_overlap_wgs84 <- nc_gages_overlap_sel %>%
  st_transform(4326)
nc_swine_overlap_wgs84 <- nc_swine_overlap %>%
  st_transform(4326)
nc_counties_overlap_wgs84 <- nc_counties_overlap%>%
  st_transform(4326)

# plot figure 1b
# add your code here
pdf(here("lessons", "sf-lesson-20200303", "outputs", "swine.pdf"), width = 11, height = 8.5)
ggmap(nc_basemap) +
  geom_sf(data = nc_state_bounds_geom_wsg84, fill = NA, lwd = 1, inherit.aes = FALSE) +
  geom_sf(data = nc_basins_overlap_sel, alpha = 0.25, lwd = 0.5, color = "black", inherit.aes = FALSE) +
  geom_sf(data = nc_gages, size = 1, inherit.aes = FALSE) +
  geom_sf(data= nc_swine, size = 1, color = "pink", inherit.aes = FALSE)+
  geom_sf(data= nc_counties_f, alpha = 0.25, color = "grey", inherit.aes = FALSE)+
  theme_bw()
# ggplot() +
