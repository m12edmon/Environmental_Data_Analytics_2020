...
\begin{verbatim}
## [1] "/Users/mashaedmondson/Environmental_Data_Analytics_2020"
\end{verbatim}
...
\texttt{\{r,\ message\ =\ FALSE\}\ \#dont\ print\ this\ on\ the\ document\ or\ print\ out\ the\ output\ \#\ Load\ package\ library(tidyverse)}
...
\CommentTok{# What happens if we don't assign a name to our imported dataset? just comes up in console }
\CommentTok{#read.csv("./Data/Raw/USGS_Site02085000_Flow_Raw.csv")}

\CommentTok{# Another option is to choose with your browser- pulls up browser and you navigate from there but not reproducable, just comment which file you are loading in }
...
practices for reproducible data analysis? \textgreater{} ANSWER: You can
find this folder within the Data folder to the metadata folder and click
on the readme text. Placement helps let us know what is within each data
set so that you can easily use data and aids in reproducability
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Rename columns - for clairty, you can ask what the columns are and then rename them (must list all column names)}
...
\KeywordTok{str}\NormalTok{(USGS.flow.data) }\CommentTok{#asking for structure of dataframe with the size, variables, vectors, and beginning entries}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{dim}\NormalTok{(USGS.flow.data) }\CommentTok{#dimensions from dataframe}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{length}\NormalTok{(USGS.flow.data) }\CommentTok{#number of columns, nrow() for how tall it is}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{class}\NormalTok{(USGS.flow.data}\OperatorTok{$}\NormalTok{datetime) }\CommentTok{#asking for specific columns}
\end{Highlighting}
\end{Shaded}
...
into R? \textgreater{} Answer: created N/A in black cells in csv files.
the blanks are characters, and numeric comes as n/a
...
\begin{quote}
Answer: Analaysis on full sets of data and correlation matrix you need
to remove the N/As but keeping N/As might show if we are missing. If you
remove the N/As it gets rid of the row and might take out other data.
\end{quote}
...
\NormalTok{USGS.flow.data}\OperatorTok{$}\NormalTok{datetime <-}\StringTok{ }\KeywordTok{as.Date}\NormalTok{(USGS.flow.data}\OperatorTok{$}\NormalTok{datetime, }\DataTypeTok{format =} \StringTok{"%m/%d/%y"}\NormalTok{) }\CommentTok{#format that the date is in code}
\end{Highlighting}
\end{Shaded}
