...
# check your date via class (EPAair$Date)
```
...
# Scatterplot
ggplot(EPAair, aes(x = Date, y = Ozone)) + #tell where we want ggplot data from
  geom_point() #use aes code to specify x axis of date and y axis of ozone, + geom_point to make a scatter plot

#plots are showing into r mark down click gear to check output in console or Inline or specify that output type in editor options at the top

O3plot <- ggplot(EPAair) + #objects are not stored automatically in Environments and this code allows it
  geom_point(aes(x = Date, y = Ozone))
print(O3plot) #print in PDF when you knit code

# Fix this code
O3plot2 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone ), color = "blue") #color needs to be outside aes() function
print(O3plot2)
...
PMplot <- 
  ...
  geom_point()
print(PMplot) #plot of PM2.5 values looking across months with PM2.5 values, and change the two different years as a different shape for each ear, and make each site a different color. 
...
print(PMplot.faceted) #seperate out sites with a facet_wrap with a number of rows and columns
...
print(PMplot.faceted2) #wrangle data without changing the dataframe, take a subset of the data frame and use subsat as a filter function to show Clemmons Middle, Leggett, bryson city. Specifcy the aes as you want your data to appear on the graph. Create a facet grid for each site listed and split them up into years (2018 and 2019)
...
PMplot.line <- 
  ggplot(subset(EPAair, Site.Name == "Leggett"), 
         aes(x = Date, y = PM2.5)) +
  geom_line()+
  #geom_point() can also place points to highlight the points of data
print(PMplot.line)
#a true time series that is continuous throughout time. Need to specifiy the same spatial construct such as subset of just sight leggett, specailize aes and create a geom_line as a continuous line to connect all the points in the data set connected by a line. can place a few N/As in the data to seperate out points on a graph visually.

# Exercise: build your own scatterplots of PeterPaul.chem.nutrients

# 1. Plot surface temperatures by day of  year. 
...
print(pp.temp)
#OR facet_wrap(vars(lakename), nrows =2)
...
print (PP.temp2) #had to create a craph like this instead of continuous because it is two lakes at multiple different depths
``` 
...
lightvsDO <- 
  ...
  geom_point()
print(lightvsDO) #looking at how light penatrates the water and dissolved oxygen

# Adjust axes
lightvsDOfixed <- 
  ggplot(PeterPaul.chem.nutrients, aes(x = irradianceWater, y = dissolvedOxygen)) +
  geom_point() +
  xlim(0, 250) + #limits for x-axis
...
tempvsdepth <- 
  ...
  #ggplot(PeterPaul.chem.nutrients, aes(x = temperature_C, y = depth, color = month)) +
  geom_point() +
  scale_y_reverse()
print(tempvsdepth) #flip scales for limnology graphs is a standard

NvsP <-
  ggplot(PeterPaul.chem.nutrients, aes(x = tp_ug, y = tn_ug, color = depth)) +
  geom_point() +
  geom_smooth(method = lm) + #lm is lineral model
  geom_abline(aes(slope = 16, intercept = 0)) #ab line to specify a line slope of your choosing, and the width around the blue line is the confidence interval -- can make that interval dissapear as method= lm, se = FALSE
print(NvsP)
...
Nutrientplot3 <- #used gathered data set and focused on lake names and concentrations and color by nutrients
  ggplot(PeterPaul.chem.nutrients.gathered, aes(x = lakename, y = concentration)) +
  geom_boxplot(aes(color = nutrient)) # Why didn't we use "fill" instead of color, we cant see what fill is for the values of nutrients are at 0
print(Nutrientplot3)
...
print(Nutrientplot4) #same data with the geom dot plot instead of a box plot-- needs more information to specifcy

# Violin plot
Nutrientplot5 <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(x = lakename, y = concentration)) +
  geom_violin(aes(color = nutrient)) #
print(Nutrientplot5) #similar to dot plot
...
print(Nutrientplot7) #wrangling data may help to ease asestics qualities of graph
...
Nutrientplot6 <-
  ...
  geom_density_ridges(aes(fill = lakename), alpha = 0.5) #alpha function showed the transparency in the graph
print(Nutrientplot6)
