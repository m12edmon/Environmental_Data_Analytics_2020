...
#not overliying trend in the data. There is no clear relationship that might obscure meaningful data. There are some high outliers and high variability among data, and missing data from 1970-1990
```
...
EnoDischarge.complete <- EnoDischarge %>%
  drop_na(discharge.mean) #remove the NAs for the missing data in the years of 1970-1990

EnoDischarge.early <- EnoDischarge.complete %>%
  filter(datetime < as.Date("1985-01-01")) #just select data from early dataset

EnoDischarge.late <- EnoDischarge.complete %>%
  filter(datetime > as.Date("1985-01-01")) #just select data from late dataset
```
...
#anything after the year 1984, 8th column has discharge mean and the frequency creates daily results. it is a time series (ts) so we dont overwrite the data and shows up in our values
```
...
#data is the actual data, remainder is leftover effects and outliers in the data because the seasonal or trend scales are realively small and its leftover data from our dataset or randomcompent of the data. 
...
# 3 coldumns refer to the seasonal, tred, and remainder. I would like to take ts and grab columns 1-3 by saying all of the rows [, and columns 1-3]. this pulls the time series into a plot. We can mutate the data set by creating observed discharge and date.
...
#assumptions for t-test well distributed across the data-set, equal variance across the two datasets. our P-value is saying that our data is not well distributed 

shapiro.test(EnoDischarge.early.subsample$discharge.mean)
shapiro.test(EnoDischarge.late.subsample$discharge.mean)
#uneuqal variance across all of our dataset so it violates our t-test and shapiro test
var.test(EnoDischarge.early$discharge.mean, EnoDischarge.late$discharge.mean)

wilcox.test(EnoDischarge.early$discharge.mean, EnoDischarge.late$discharge.mean) #sig difference in disrcharge across the dataset
...
> We would used the summary function to better understand our data. From that summary we would have a two frame graph with the same axes. or use a blox plot/ violin/density plot.
Mean daily dischare in the Eno River from 1927- 1971 as significantly higher than mean daily discharge from 1985-2019 (Wilcox testm W= 1.17*10^8, p<0.0001).
...
> aggregation: Splits the data into subsets, computes summary statistics for each, and returns the result in a convenient form.

> subsampling: Perform subsampling at multiple proportions on a matrix of count data representing mapped reads across multiple samples in many genes. For each sample, perform some statistical operations. Used the least but can be very useful in certain situations.

> interpolation: For a given set of x values, this function will return the corresponding interpolated values. Helping to fill in gaps in data to generate an equal distribution based on your own data. (Like in GIS)
...
# na.approx function fills in NAs with a linear interpolation (another option)
# Spline interpolation can also be specified as an alternative (another option)
# Piecewise constant interpolation can be done with na.aggregate (another option)
NCAir.Garinger$Ozone <- na.approx(NCAir.Garinger$Ozone)
NCAir.Garinger$PM2.5 <- na.approx(NCAir.Garinger$PM2.5) #to fill in NAs for Ozone and PM2.5 columns, but had to do it seperately 
...
#need to wrangle data to use aggregate data by month and get one value by aggregation
EnoDischarge.late.monthly$Date <- as.Date(paste(EnoDischarge.late.monthly$Year, 
                                                EnoDischarge.late.monthly$Month, 
                                                1, sep="-"), 
                                          format = "%Y-%m-%d") #one discharge measurement per month for 35 years, but we want a date to graph things for the first of the month. Create a new column for date monthly$Date, then create a date, use the year and month and take the first day of the year in the format of year-month-day
...
summary(EnoDischarge.late.trend) #null hypothesis is not a trend overtime and the p value is high and a z score close to 0 meaning we cannot discern a specific trend acrossour data set. Z score of  -.96 or more negative or positive .96or higher is significant
...
> There is no significant trend across the seasons in our data set for discharge over 35 years. There is a lot of variablity over time, but no major trends within the Eno River (Z = -0.1396, P <0.8889).
