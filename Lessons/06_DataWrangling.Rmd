...
> Add notes here: To measure how they change overtime with the amount of temperature, oxygen, and light to show how it changes over the year, size of lakes, how they could be changing over time. It can show trends that you might not have been able to see over long term monitoring. Measure porducitity of lakes. Ecosystem health and potenial runoff. Range of tolerance of organisms.
...
head(NTL.phys.data) #first six rows of data
summary(NTL.phys.data)
str(NTL.phys.data) #structure- types of data/vector
dim(NTL.phys.data) #dimensions

class(NTL.phys.data$sampledate)
# Format sampledate as date
NTL.phys.data$sampledate <- as.Date(NTL.phys.data$sampledate, format = "%m/%d/%y") #verify date is formatted correctly
```

## Data Wrangling

Data wrangling extends data exploration: it allows you to process data in ways that are useful for you. An important part of data wrangling is creating *tidy datasets*, with the following rules: (tindy datasets are easily manipulated)
...
```{r, results = "hide"} 
vignette("dplyr") #vignette- goes through the main functions, tips and tricks, functions, and how to use it. the {"hide"} means it wont show up when you knit it
```
...
`==` #equals - factor or numeric values
`!=` #not equal to
`<` #less than
`<=` #less than or eaual too
`>` #greater than
...
class(NTL.phys.data$depth) #depth in meters

# matrix filtering
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,] #take this datafram and within the matrix we will take the data where depth is equal to 0. In matrix notation we need to specify both rows and columns hence ","

# dplyr filtering
NTL.phys.data.surface2 <- filter(NTL.phys.data, depth == 0) #make new option in filter function, where to grab that data from, and logical statement on what to classify. We want it to grab the NTL data where depth =0 
NTL.phys.data.surface3 <- filter(NTL.phys.data, depth < 0.25) #want all the depths less than 0.25meters
...
summary(NTL.phys.data$lakename) #how many rows each lake represented. Peter and paul lake are the most sampled
NTL.phys.data.PeterPaul1 <- filter(NTL.phys.data, lakename == "Paul Lake" | lakename == "Peter Lake") #used or to have paul or peter lake to get samples from
NTL.phys.data.PeterPaul2 <- filter(NTL.phys.data, lakename != "Central Long Lake" & 
                                     ...
                                     lakename != "Ward Lake" & lakename != "West Long Lake") #specifiy all lakes that are not peter or paul lake. Another way to do the statement above
NTL.phys.data.PeterPaul3 <- filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake")) # %in% c() want dataset to inclde any data with paul and peter lake

# Choose a range of conditions of a numeric or integer variable
summary(NTL.phys.data$daynum) #range of data where 55 represents 55th day of year and day 307 near the end of the year- not sampling when the lakes are frozen 
NTL.phys.data.JunethruOctober1 <- filter(NTL.phys.data, daynum > 151 & daynum < 305) #select dates from june to oct.
NTL.phys.data.JunethruOctober2 <- filter(NTL.phys.data, daynum > 151, daynum < 305) #same thing , & 
NTL.phys.data.JunethruOctober3 <- filter(NTL.phys.data, daynum >= 152 & daynum <= 304) #same thing
NTL.phys.data.JunethruOctober4 <- filter(NTL.phys.data, daynum %in% c(152:304)) #same thing filtering differently
...
> Answer: more difficult/work and you want it to filter by attributes. If the dataset changes the rows could not be the same and your code wouldn't work.
...
```{r}
NTL.phys.data.depth.ascending <- arrange(NTL.phys.data, depth) #look at specific piece and arrange it by depth
NTL.phys.data.depth.descending <- arrange(NTL.phys.data, desc(depth)) #desc function for descending
...
#shallow depths have higher temperatures
```
### Select
...
```{r}
NTL.phys.data.temps <- select(NTL.phys.data, lakename, sampledate:temperature_C) #name which column we want to keep or leave out of the dataset. Want to select NTL data and want the lakename, sampledate through temperature. This is different than the filter function -- only selecting based on the same of the column not by selecting features; filter is horizontal and select is vertical; commas between them instead of logical statements; select you can say you don't want to include something with a -"column name"
...
NTL.phys.data.temps <- mutate(NTL.phys.data.temps, temperature_F = (temperature_C*9/5) + 32) #add a column to the end of your dataframe; specify we want to compute the temp into Ferenihite
...
NTL.phys.data.PeterPaul1 <- select(NTL.phys.data.PeterPaul1, lakeid:daynum, month, sampledate:comments) #placing month in the middle of the dataset next to the date

# find out the start and end dates of the dataset
interval(NTL.phys.data.PeterPaul1$sampledate[1], NTL.phys.data.PeterPaul1$sampledate[21613]) #intervals btw sampling dates
...
NTL.phys.data.processed <- 
  NTL.phys.data %>% #selecting data for a pipe, the pipe function means "then" and filtering for lakes then select certain columns then mutate the columns with a new column to create a temp in ferienheight
...
```{r}
write.csv(NTL.phys.data.PeterPaul1, row.names = FALSE, file = "./Data/Processed/NTL-LTER_Lake_ChemistryPhysics_PeterPaul_Processed.csv") #most of the time kateri will save processed data
