...
```{r, message = FALSE} #dont print this on the document or print out the output
# Load package
library(tidyverse)
```
...
# What happens if we don't assign a name to our imported dataset? just comes up in console 
#read.csv("./Data/Raw/USGS_Site02085000_Flow_Raw.csv")

# Another option is to choose with your browser- pulls up browser and you navigate from there but not reproducable, just comment which file you are loading in 
...
> ANSWER: You can find this folder within the Data folder to the metadata folder and click on the readme text. Placement helps let us know what is within each data set so that you can easily use data and aids in reproducability
...
# Rename columns - for clairty, you can ask what the columns are and then rename them (must list all column names)
...
str(USGS.flow.data) #asking for structure of dataframe with the size, variables, vectors, and beginning entries
dim(USGS.flow.data) #dimensions from dataframe
length(USGS.flow.data) #number of columns, nrow() for how tall it is
...
class(USGS.flow.data$datetime) #asking for specific columns
...
> Answer: created N/A in black cells in csv files. the blanks are characters, and numeric comes as n/a
...
> Answer: Analaysis on full sets of data and correlation matrix you need to remove the N/As but keeping N/As might show if we are missing. If you remove the N/As it gets rid of the row and might take out other data. 
...
USGS.flow.data$datetime <- as.Date(USGS.flow.data$datetime, format = "%m/%d/%y") #format that the date is in code
```
