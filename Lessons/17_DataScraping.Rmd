...
library(rvest)
#install.packages("ggrepel") - labeling function to not overlap
library(ggrepel)
...
* Manual copy-paste
* API (retrieve data from standard code; data must be in prescribed format) - extra set of code that allows you to access codes inside in an analyzable form (such as Facebook)
* DOM parsing
...
```{r}
# Specify website to be scraped you want to interface it-- call it url
url <- "https://www.epa.gov/nutrient-policy-data/waters-assessed-impaired-due-nutrient-related-causes"

# Reading the HTML code from the website-- creating new object to read url
...
State <- webpage %>% html_nodes("table:nth-child(20) td:nth-child(1)") %>% html_text() #read in URL
Estuaries.Assessed.mi2 <- webpage %>% html_nodes("table:nth-child(20) td:nth-child(2)") %>% html_text() #create pipe
...
#read in webpage, find specific nodes, then read in html text
#visit webpage, find the nodes, and read this in as text. Note: each of these objects has 23 items in it; consistency is good! (If lengths were not consistent you could use matrix subsetting to make consistent)
...
> Some states haven't done many TMDLS and dont seem to be that impared. No specific information from the site if there was total estuaries assess what percent of them were impared and how many have TMDLs. Delaware was an outlier and has a high amount of impairement with lowest assessment. 
...
> Those have N/A values. This is because we replaced +/- with nothing in R and that automaticlly created NAs.
