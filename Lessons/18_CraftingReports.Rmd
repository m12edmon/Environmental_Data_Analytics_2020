...
```{r, message = FALSE}
LTER_Lake_Nutrients <- read.csv("./Data/Raw/NTL-LTER_Lake_Nutrients_Raw.csv")
LTER_Lake_Nutrients$sampledate <- as.Date(LTER_Lake_Nutrients$sampledate, format = "%m-%d-%Y")
...
Nutrients_summary.tp <- LTER_Lake_Nutrients %>%
   group_by(lakename) %>%
...
library(scales)
ggplot(LTER_Lake_Nutrients, aes(x = as.POSIXct(sampledate), y = tn_ug, color = lakename)) +
  geom_point() +
...
```{r, include=FALSE, fig.height = 4, fig.width = 10}
ggplot(LTER_Lake_Nutrients, aes(x = as.POSIXct(sampledate), y = tp_ug, color = lakename)) +
  geom_point() +
...
> ANSWER: * suppress display of warnings - warnings=FALSE
