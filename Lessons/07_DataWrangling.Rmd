...
# Select only Peter and Paul Lakes from the nutrient dataset
NTL.nutrient.data.PeterPaul <- filter(NTL.nutrient.data, lakename == "Paul Lake" | lakename == "Peter Lake")

NTL.nutrient.data.PeterPaul <-
  NTL.nutrient.data %>%
  filter(lakename == "Paul Lake" | lakename == "Peter Lake") %>% #if we wanted to read it in as a Pipe
  droplevels()
...
NTL.nutrient.data.PeterPaul <- droplevels(NTL.nutrient.data.PeterPaul) #remove other lakes that arn't peter & paul
summary(NTL.nutrient.data.PeterPaul$lakename)
...
NTL.nutrient.data.PeterPaul <- 
  ...
  drop_na(depth) #another way to do what was done above, and drop any NA values for the depth column
...
NTL.nutrient.data.PeterPaul.subset <- select(NTL.nutrient.data.PeterPaul, lakename: sampledate, depth:po4) #or
NTL.nutrient.data.PeterPaul.subset <- select(NTL.nutrient.data.PeterPaul, -lakeid, -depth_id, - comments) #or a pipe function not listed
```
...
Note: `tidyr` is moving away from `gather` and `spread` and toward `pivot_longer` and `pivot_wider`, respectively (Newly development). Note that the latter functions are only available on the newest version of `tidyr`, so we are using `spread` (wide form dataspread) and `gather`(long form dataset) today to ensure compatibility. `gather` and `spread` are not going away, but they are not under active development.

```{r}
# Gather nutrient data into one column
NTL.nutrient.data.PeterPaul.gathered <- gather(NTL.nutrient.data.PeterPaul.subset, "nutrient", "concentration", tn_ug:po4) #gather nutrient subset data and supply what the new columns will be- calling it nutrient, use the concentration and we will use total nitrogran all the way through 5 columns as data

NTL.nutrient.data.PeterPaul.gathered <- subset(NTL.nutrient.data.PeterPaul.gathered, !is.na(concentration))
count(NTL.nutrient.data.PeterPaul.gathered, nutrient) #remove NAs in the concentration column
...
# Spread nutrient data into separate columns (opposite of gather, just supply which one will be the column names and the value of each of those)
...
# Opposite of 'separate' is 'unite' -- you can use for dates but lubridate is preferred
...
#long format may be better used if you wanted to plot nutrient concentrations but have them all be a different color. Very useful for vizualization on the long format one
```
...
```{r}
#kateri uses left_join and full_join the most
NTL.phys.nutrient.data.PeterPaul <- full_join(NTL.phys.data.PeterPaul.subset, NTL.nutrient.data.PeterPaul.subset)
#keep every observation- physically join the nutrient dataset and the peterbaul subset and look for data that matches. Matches based on the content but finds it by column names
...
#read each of these in seperately from the raw data- choosing the first 3
Litter.2019 <- rbind(Litter.June2016, Litter.July2016, Litter.August2016) #binding all the rows of the 3 column datasets above together
```
...
LitterFiles
#list.files you can tell R to look for the pattern where any of the sets have .cvs and use the full names. Had 20 files in that folder and listed them
Litter <- LitterFiles %>%
  ldply(read.csv) #make a new dataset with the pipe function and creates a Litter data frame all into one
```
...
LitterTrap <- left_join(Litter, Trap, by = c("siteID", "plotID", "trapID")) #left join of data and place information from Trap into the Litter data
...
NTL.PeterPaul.summaries <- 
  ...
  summarise(meantemp = mean(temperature_C), 
            ...
            meanTP = mean(tp_ug), 
            sdTP = sd(tp_ug))
#creates a data summary of lake name, month, mean temp, sd temp, mean total nitrogen, sd tn, mean tp, sd of total phos. 
#understand summaries of our data. Taking the datasent of nutrient data of peter and paul that we just joined, we want to filter the surfact data dpeth == 0, and group data by month for seasonal changes, get ride of NAs because we need a mathmaticial operation, then we asked R to summarise data- and only complete cases. Our pipe selected groups of months and got mean temps across the dataset. 
