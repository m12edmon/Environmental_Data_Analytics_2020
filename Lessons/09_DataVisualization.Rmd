...
### Themes- premade ggplot themes for visulaizations
...
O3plot2 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_bw() #dark line sound graph and light gray lines for axis
print(O3plot2)

O3plot3 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_light() #light gray line as border with gray lines for axis
print(O3plot3)

O3plot4 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_classic() #Classic black boarder and no grid in the white background
print(O3plot4)
```
...
```{r, fig.height = 3, fig.width = 4, warning = FALSE}
mytheme <- theme_classic(base_size = 14) + #make your own theme-starting with teme classic and axis size
  theme(axis.text = element_text(color = "black"),  #make black text with legend position at top
...
theme_set(mytheme) #theme_set and set it as my theme to keep things similar across themes
...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  geom_hline(yintercept = 50, lty = 2) + #horizontal line for y intercept, and lty is line typed = dash
  geom_hline(yintercept = 100, lty = 2) + #adding another horizontal line for y intercept-- add this first so points can show up over lines
  geom_point(alpha = 0.5, size = 1.5) + # alpha makes things transparent 50% and created point size
  geom_text(x = as.Date("2020-01-01"), y = 45, label = "good", hjust = 1, fontface = "bold") + #text and need to specify where you want it on the graph with x and y coordinates-- hjust is on the right side
  ...
  scale_x_date(limits = as.Date(c("2018-01-01", "2019-12-31")), #scaled the x axis to bound dates
    date_breaks = "2 months", date_labels = "%b %y") + #making breaks for 2 month intervals and tell the labels as month and date-- "b" stands for 3 letter month "B" is whole month spelled out
  ylab(expression("O"[3]* " AQI Value")) + #creating an Y axis title with a subscript-- * tells R you are finished with your subscript
  theme(axis.text.x = element_text(angle = 45,  hjust = 1)) #preventing overlapping of text
print(O3plot7)  
...
NvsP2 <-
 ...
 labs(x= "", y= expression(paste("Concentration", mu, "g/L"))) +
  #     y= expression(paste("TP""(", mu, "g/L"))), + #~(mu) creates micograms symbol
  #color = "Depth (m)", shape = ""
  scale_shape_manual(values = c(15, 17)) +
  #scale_color_distiller(palette = "Blues", direction = 1) + # use scale_color_brewer for discrete variables
  scale_color_viridis(option = "magma", direction = -1, end=0.8) + #end = 0.8 chops off color ramp at end
  theme(legend.position = "right", #placing the legend on the right of the graph
