...
#this is telling us that slope (even if they have different axis) is the same. At paul lake at 0 meters of depth has intercept of 284.437. Peter lake is sig different 284+66= 350 intercept. For each meter we go in depth TN will increase by 68 micro litters.


# interaction effects
TNancova.interaction <- lm(data = PeterPaul.chem.nutrients, tn_ug ~ lakename * depth)
summary(TNancova.interaction)
#this is terlling us we might have different intercepts and different slopes for each lake
TNplot <- ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = tn_ug, color = lakename)) + 
  geom_point() + 
  labs(x= "Depth (m)", y = expression(TN ~ (mu*g/L)), color = "")+
  #scale_color_brewer(palete = "Dark 2")
  scale_color_manual(values = c("darkblue", "red"))+
  geom_smooth(method = "lm", se = FALSE) +
  xlim(0, 10)
print(TNplot)
#need to put a line of best fit in manually with this specific test
```
...
NTL.summertemp <- 
  ...
  na.exclude()
#use space as our indenpendent variables. We are interested how lakes in the general area have responded to climate change and use the variablity across the models to portray that.
...
```{r}
TempTest.mixed <- lme(data = NTL.summertemp, #lme = lineral mixed effect
                     temperature_C ~ year4, 
                     random = ~1|lakename) #grouped by lakename
summary(TempTest.mixed) #interested in incorperating the effect into the model
rsquared(TempTest.mixed) #when adding in lake as a random effect there was a margin of 66 and condition of 11. Not sig enough for us to use this in our model
...
anova(TempTest.mixed, TempTest.fixed) #testing if there is a sig difference between the models
...
ggplot(NTL.summertemp, aes(x = year4, y = temperature_C, color = lakename)) +
  geom_point(e) +
  scale_color_viridis_d() +
  #geom_smooth(method = "lm", se = FALSE) this will produce line of best fit from the first model that is not a lineral relationship it is representing the fixed effect model
  geom_abline(intercept = -97.72, slope = 0.06) + #taken from the model we ran
  # make it look better
...
> ANSWER: Our slope is significant therefore there is a significant change in temperature. We are generally seeing a 0.6 degree celcius change per year from each lake in Wisconsin. It means the lake temp is 1.8 degress F over the course of 30 years. 
