...
```{r check.pdf}
pdf(here("lessons", "sf-lesson-20200303", "outputs", "pdf_test.pdf"), width = 11, height = 8.5)
ggplot(data = cars) +
  geom_point(aes(x = dist, y = speed))
dev.off()
# it's ok to get a "null device" warning here. Here we tested to see if the PDF works. It take awhile for it to show up in the plots pane so it is easier to save it as a PDF and look at it there.
```
...
1. [https://epsg.io/](http://www.epsg-registry.org/) - open source nicer user interface to access data in the EPSG registry, includes a little transformation calculator too! and proj4 string generator, this is my go-to CRS look-up database ***kateri preferred
...
southeast_state_bounds_geometry <- st_geometry(southeast_state_bounds) #grabs just geometry
...
# it's an sfc class, specifically an sfc_MULTIPOLYGON class no longer a dataframe
```
...
# select a state, here I'm choosing Sorth Carolina (SC)
# add your code here
sc_state_bounds_geom <- southeast_state_bounds %>%
  filter(NAME == "Sorth Carolina") %>%
  st_geometry()
...
# bonus
# add your code here
sc_area <- st_area(sc_state_bounds_geom) # find the area of NC
sc_centroid <- st_centroid(sc_state_bounds_geom) # find the centroid of the NC
sc_gages_buffer_100m <- st_buffer(sc_gages, dist = 100) # make a buffer around the stream gages in NC
not_sc_gages <- st_difference(gages, sc_state_bounds_geom) # find stream gages not in NC
```
...
pdf(here("lessons", "sf-lesson-20200303", "outputs", "southeast_state_bounds_by_area.pdf"), width = 11, height = 8.5)
...
ggplot() +
  geom_sf(data = sc_state_bounds_geom, fill = NA) +
  geom_sf(data = sc_basins_nf_seplains, alpha = 0.25) +
...
sc_bbox_fix <- c(left = nc_bbox[[1]], bottom = nc_bbox[[2]], right = nc_bbox[[3]], top = nc_bbox[[4]])
...
pdf(here("lessons", "sf-lesson-20200303", "outputs", "sc_without_basemap.pdf"), width = 11, height = 8.5)
ggplot() +
...
# plot with basemap
# windows version?
pdf(here("lessons", "sf-lesson-20200303", "outputs", "sc_with_basemap.pdf"), width = 11, height = 8.5)
ggmap(sc_basemap) +
...
# make figure 1b
# find watersheds that overlap with SC (want the whole watershed, not just the part of the watershe that's inside the NC bounds)
# add your code here
sc_basins_overlap_binary <- basins_nf_seplains %>%
  st_overlaps(sc_state_bounds_geom)

# look at result
# add your code here
head(sc_basins_overlap_binary)
class(sc_basins_overlap_binary)
...
# add your code here
sc_basins_overlap_logic <- sc_basins_overlap_binary %>%
  map_lgl(~ binary_logic_converter(.x))

# look at result
# add your code here
head(sc_basins_overlap_logic)

# use nc_basins_overlap_logic to select only watersheds that overlap
# add your code here
sc_basins_overlap_sel <- basins_nf_seplains[sc_basins_overlap_logic, ]

# check class
# add your code here
class(sc_basins_overlap_sel)
...
# add your code here
sc_gages_overlap_sel <- southeast_gages %>%
  st_intersection(sc_basins_overlap_sel)

# convert all to WGS84
# add your code here
sc_state_bounds_geom_wsg84 <- sc_state_bounds_geom %>%
  st_transform(4326)
sc_basins_overlap_wgs84 <- sc_basins_overlap_sel %>%
  st_transform(4326)
sc_gages_overlap_wgs84 <- sc_gages_overlap_sel %>%
  st_transform(4326)

# plot figure 1b
# add your code here
pdf(here("lessons", "sf-lesson-20200303", "outputs", "figure_SC.pdf"), width = 11, height = 8.5)
ggmap(sc_basemap) +
  geom_sf(data = sc_state_bounds_geom_wsg84, fill = NA, lwd = 1, inherit.aes = FALSE) +
  geom_sf(data = sc_basins_overlap_sel, alpha = 0.25, lwd = 0.5, color = "black", inherit.aes = FALSE) +
  geom_sf(data = sc_gages_overlap_sel, size = 1, inherit.aes = FALSE) +
  theme_bw()+
  labs(x = "Longitude", y = "Latitude") +
   ggtitle("Map of South Carolina's Overlapping Watersheds and Stream Gages")
dev.off()
...
pdf(here("lessons", "sf-lesson-20200303", "outputs", "nc_gages_by_tabular_data.pdf"), width = 11, height = 8.5)
ggplot() +
...
# plot
#add your code here
pdf(here("lessons", "sf-lesson-20200303", "outputs", "nc_basins_by_tabular_data.pdf"), width = 11, height = 8.5)
ggplot() +
...
st_write(nc_state_bounds_geom, here("lessons", "sf-lesson-20200303", "outputs", "nc_boundary.shp"))
```
