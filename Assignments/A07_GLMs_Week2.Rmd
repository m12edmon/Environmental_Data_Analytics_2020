...
NTL_LTER_Chem_Raw <- read_csv("./Data/Raw/NTL-LTER_Lake_ChemistryPhysics_Raw.csv")
NTL_LTER_Litter<- read_csv("./Data/Processed/NEON_NIWO_Litter_mass_trap_Processed.csv")
...
NTLAIC <- lm(data = NTL.July.AIC, temperature_C~ year4 + daynum + depth) 
step(NTLAIC) 
...
#6
NTL.ancova.main <- lm(data = NTLmodel, temperature_C ~ depth*lakename)
summary(NTL.ancova.main) 

NTL.ancova.interaction <- lm(data = NTLmodel, temperature_C ~ depth*lakename)
...
```{r}
#8 - what is the data here
ANCOVAplot <- ggplot(NTL.July, aes(x = temperature_C, y = depth, color = lakename)) + 
  geom_point() + 
...
```{r}
#c.
litter_mixed <- lme(data = NTL_LTER_Litter,
                           dryMass ~ nlcdClass * functionalGroup)
