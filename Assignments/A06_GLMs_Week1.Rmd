...
```{r}
#not keeping Peter and Paul lake? 
...
> Answer: T-test is one predictable variable (would be good for lakes only). We would use the ANOVA GLM application. We would pick two way anova because we have two varibales. TN = lak+ month+ error  with interaction. Though the lakes might be similar one is being dumped with nutrients so the interaction might be able to determine that.
...
summary(TN.anova.2way) #proceede with main interaction. Only sig differenceis peter and paul lake - graph is and designate peter lake vs paul lake and peter lake is bigger because it is 384 vs 300

# Format as lm
#TN.anova.2way2 <- lm(data = Depths.Totals, tn_ug ~ month * lakename)
