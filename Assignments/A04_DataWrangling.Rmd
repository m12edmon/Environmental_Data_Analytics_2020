...
```{r}
#3?????????????????????? and did I select #4 properly as the way it should be graded
...
EPAair_O3_PM25_NC1718 <- 
  EPAair_O3_PM25_NC %>%
  filter("Linville Falls", "Durham Armory", "Leggett", "Hattie Avenue", "Clemmons Middle", "Mendenhall School", "Frying Pan Mountain", "West Johnston Co.", "Garinger High School", "Castle Hayne", "Pitt Agri. Center", "Bryson City", "Millbrook School")%>%
  group_by(date, Site.Name, AQS_PARAMETER_DESC, COUNTY) %>%
  summarise(mean_AQI_value = mean(DAILY_AQI_VALUE), 
            meanLat = mean(SITE_LATITUDE), 
            meanLong = mean(SITE_LONGITUDE))

EPAair_O3_PM25_NC1718.dates <- mutate(EPAair_O3_PM25_NC1718, Month = month(Date))
EPAair_O3_PM25_NC1718.dates <- mutate(EPAair_O3_PM25_NC1718, Year = year(Date))
EPAair_O3_PM25_NC1718.dates <- select(EPAair_O3_PM25_NC1718, date, month, year, DAILY_AQI_VALUE:SITE_LONGITUDE)
dim(EPAair_O3_PM25_NC1718.dates)     
dim(EPAair_O3_PM25_NC1718)     

#9
EPAair_O3_PM25_NC1718.spread <- spread(EPAair_O3_PM25_NC1718, Ozone, PM2.5, DAILY_AQI_VALUE)
